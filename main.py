...
def nmToRGB_v2(wavelength):
    """Convertit une longueur d'onde (nm) en valeurs RGB avec une intensité adaptée."""
    red, green, blue = 0.0, 0.0, 0.0
    intensity = 1.0  # Facteur de luminosité

    if 380 <= wavelength < 440:
        red = -(wavelength - 440) / (440 - 380)
        blue = 1.0
        intensity = 0.3 + 0.7 * (wavelength - 380) / (440 - 380)  # Ajustement de l'intensité
        ...
        red = 1.0
        intensity = 0.3 + 0.7 * (780 - wavelength) / (780 - 645)  # Ajustement pour les longues longueurs d'onde

    # Appliquer la correction gamma (approx. 0.8 pour correspondre à la perception humaine)
    gamma = 0.8
    red = (red * intensity) ** gamma
    green = (green * intensity) ** gamma
    blue = (blue * intensity) ** gamma

    # Clamping pour éviter des valeurs hors limites
...
# Test de la fonction avec une plage de longueurs d'onde
image_RVB = calcule_true_rgb(402, 998 )
